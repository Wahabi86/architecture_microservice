...
function MyList() {
  ...
  useEffect(() => {
    const fetchWatchlistDetails = async () => {
      ...
      try {
        ...
        setError("Gagal memuat daftar film Anda. Silakan coba lagi.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleRemove = async (id) => {
    try {
      ...
      setMyListMovies((prevList) => prevList.filter((movie) => movie.id !== id));
      ...
      alert("Gagal menghapus film dari daftar Anda. Silakan coba lagi.");
    }
  };
  ...
  return (
    <div className="px-20 pt-28 text-white">
      ...
      {!isLoading && !error && myListMovies.length === 0 && (
        <p className="text-gray-400 text-center py-10 text-lg">You haven't added any movies to your list yet.</p>
      )}
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-4 mt-8 mb-12">
              <button
                ...
                className="text-[#00BFFF] disabled:opacity-40 hover:text-white transition-opacity"
                ...
                {Array.from({ length: totalPages }, (_, index) => (
                  <button
                    ...
                    className={`w-3 h-3 rounded-full transition-colors duration-200 ${
                      currentPage === index + 1 ? 'bg-[#00BFFF]' : 'bg-gray-400 hover:bg-gray-300'
                    }`}
