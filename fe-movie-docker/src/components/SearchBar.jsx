...
function SearchBar() {
  ...
  const handleSearch = (e) => {
    ...
    if (value.trim() === "") {
      ...
      const results = allMovies.filter((movie) => movie.title.toLowerCase().includes(value));
  ...
  return (
    <div className="relative w-65">
      ...
      {isOpen && searchResults.length > 0 && (
        <div
          className="absolute top-full mt-2 w-full bg-gray-900 border border-[#00BFFF] rounded-lg shadow-lg max-h-96 overflow-y-auto z-50 
  scrollbar-hide"
        >
      ...
      {/* Info jika tidak menemukan movie*/}
      {isOpen && searchInput && searchResults.length === 0 && <div className="absolute top-full mt-2 w-full bg-gray-900 border border-[#00BFFF] rounded-lg p-4 text-center text-gray-400 z-50">No movies found</div>}
    </div>
  );
}
